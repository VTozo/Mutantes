...
class Territorio extends JPanel {
    ...
    private void detectar_colisoes() {
        ...
        for (int i = 0; i < seres.size(); i++) {
            ...
            for (int c = 0; c < seres.size(); c++) {
                if (i != c) {
                    Ser s2 = seres.get(c);
                    if (s1.getRetangulo().intersects(s2.getRetangulo()) && s1.getPosicao() != s2.getPosicao()) {
                        fusao(i, c);
                        ociosidade = 100;
                        break;
                    }
    ...
    private void gerar_seres(int n) {
        ...
        while (seres_gerados < n) {
            ...
            if (Math.sqrt(Math.pow(ser.getPosicao()[0] - racional.getPosicao()[0], 2) + Math.pow(ser.getPosicao()[1] - racional.getPosicao()[1], 2)) > 200) {
                seres.add(ser);
                seres_gerados++;
            }
        }
